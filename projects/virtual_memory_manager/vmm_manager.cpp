...
namespace vmm
{
    vmm_manager::vmm_manager(size_t page_count, std::string file_path): 
        file_path(file_path), page_count(page_count), file(file_path)
    { 
    ...
    long int vmm_manager::lookup(std::string id)
    {
        ...
        for(size_t i = 0; i < p_pages.size(); i++)
        {
            if(p_pages[i].id == id)
            {
                ...
                this->log(
                    "Memory Manager, SWAP: Variable " + p_pages[i].id + " with Variable " + p.id
                );
